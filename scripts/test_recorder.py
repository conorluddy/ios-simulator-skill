...
class TestRecorder:
    ...
    def __init__(self, test_name: str, output_dir: str = "test-artifacts", udid: Optional[str] = None):
        """
    ...
    def step(self, description: str, assertion: Optional[str] = None, metadata: Optional[Dict] = None):
        """
    ...
    def generate_report(self) -> Dict[str, str]:
        ...
        with open(report_path, 'w') as f:
            ...
            for step in self.steps:
                f.write(f"### Step {step['number']}: {step['description']} ({step['timestamp']:.1f}s)\n\n")
...
def main():
    ...
    parser.add_argument(
        '--test-name',
        required=True,
        help='Name of the test being recorded'
    )
    parser.add_argument(
        '--output',
    ...
    parser.add_argument(
        '--udid',
        help='Device UDID (uses booted if not specified)'
    )
