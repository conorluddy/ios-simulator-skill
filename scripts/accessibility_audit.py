...
class AccessibilityAuditor:
    ...
    CRITICAL_RULES = {
        ...
        'empty_button': lambda e: e.get('type') == 'Button' and not (e.get('AXLabel') or e.get('AXValue')),
...
def main():
    ...
    parser.add_argument(
        '--udid',
        help='Device UDID (uses booted device if not specified)'
    )
    parser.add_argument(
        '--output',
    ...
    parser.add_argument(
        '--verbose',
        action='store_true',
        help='Include all issue details (increases output)'
    )
        ...
        if args.verbose:
            ...
            if result.get('top_issues'):
                ...
                for issue in result['top_issues']:
                    print(f"  [{issue['severity']}] {issue['rule']} ({issue['count']}x) - {issue['fix']}")
